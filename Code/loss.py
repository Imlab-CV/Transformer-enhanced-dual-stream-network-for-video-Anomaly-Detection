...
"""
                    #for Ped_dataset and Avenue_dataset
def MIL(y_pred, batch_size, is_transformer=0):
    ...
    for i in range(batch_size):
        ...
        y_anomaly = y_pred[i, :6]#[anomaly_index]
        y_normal  = y_pred[i, 6:]#[normal_index]
        ...
        smooth += torch.sum((y_pred[i,:6] - y_pred[i,1:7])**2)*0.00008
    loss = (loss+sparsity+smooth)/batch_size

    return loss


"""             #for UCF & Shengaitech_dataset
def MIL(y_pred, batch_size, is_transformer=0):
    loss = torch.tensor(0.).cuda()
    loss_intra = torch.tensor(0.).cuda()
    sparsity = torch.tensor(0.).cuda()
    ...
    for i in range(batch_size):
        ...
        sparsity += torch.sum(y_anomaly)*0.00008
        smooth += torch.sum((y_pred[i,:32] - y_pred[i,1:33])**2)*0.00008
    loss = (loss+sparsity+smooth)/batch_size
